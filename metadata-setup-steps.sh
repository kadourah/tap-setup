nslookup metadata-store.tap.tanzu.kadourah.com
curl https://metadata-store.tap.tanzu.kadourah.com/api/health -k -v

kubectl get secret ingress-cert -n metadata-store -o json | jq -r '.data."ca.crt"' | base64 -d > insight-ca.crt
tanzu insight config set-target https://metadata-store.tap.tanzu.kadourah.com --ca-cert insight-ca.crt
